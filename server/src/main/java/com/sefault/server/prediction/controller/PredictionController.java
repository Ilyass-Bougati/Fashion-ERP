...
@Tag(name = "Machine Learning Predictions", description = "Read-only endpoints for retrieving AI-generated business forecasts (powered by TimesFM).")
public class PredictionController {
    ...
    @Operation(
            ...
            description = "Retrieves predicted daily store performance metrics, including expected Net Revenue, Units Sold, and Total Transactions. Each metric includes a 10th-90th percentile confidence interval (Lower and Upper bounds)."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Successfully retrieved a paginated list of sales predictions"),
    ...
    public ResponseEntity<Page<SalesPredictionRecord>> getSalesPredictions(
            @Parameter(description = "Fetch predictions starting from this date (inclusive). Usually set to today's date.", example = "2026-05-12")
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,

            @Parameter(description = "The aggregation period. Expected value is DAILY.", example = "DAILY")
            @RequestParam PeriodType periodType,

            @Parameter(description = "Pagination parameters (page number, size). Default size is 30 (1 month forecast).")
    ...
    @Operation(
            ...
            description = "Retrieves the predicted depletion of stock levels for a specific product variation over the coming days. Critical for automated reordering and avoiding stockouts."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Successfully retrieved stock predictions for the requested SKU"),
            @ApiResponse(responseCode = "400", description = "Invalid date format, missing parameters, or missing SKU", content = @Content)
    })
    @GetMapping("/stock")
    ...
    @Operation(
            ...
            description = "Retrieves high-level quarterly/annual predictions for Total Corporate Revenue and Net Profit."
    )
    @ApiResponses({
    ...
    public ResponseEntity<Page<FinancialPredictionRecord>> getFinancialPredictions(
            @Parameter(description = "Fetch predictions starting from this date. Usually the 1st of the current month.", example = "2026-05-01")
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,
    ...
    @Operation(
            ...
            description = "Retrieves the expected Gross Sales volume generated by a specific employee. Used for setting quotas and estimating future commissions."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Successfully retrieved performance predictions for the employee"),
    ...
    public ResponseEntity<Page<EmployeePerformancePredictionRecord>> getEmployeePredictions(
            ...
            @Parameter(description = "The unique Carte d'Identité Nationale (CIN) of the employee.", example = "CIN-SEED-EMP")
            @RequestParam String cin,

            @Parameter(description = "Pagination parameters. Default size is 30.")
            @PageableDefault(size = 30) Pageable pageable) {
