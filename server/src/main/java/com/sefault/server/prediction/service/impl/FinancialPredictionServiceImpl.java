...
public class FinancialPredictionServiceImpl implements FinancialPredictionService {
    ...
    public void generateMonthlyFinancialForecast() {
        List<FinancialStat> history =
                financialStatRepository.findTop24ByPeriodTypeOrderByStatDateDesc(PeriodType.MONTHLY);
        ...
        List<Double> revenueHistory =
                chronological.stream().map(FinancialStat::getTotalRevenue).toList();
        List<Double> profitHistory =
                chronological.stream().map(FinancialStat::getNetProfit).toList();
        ...
        for (int i = 0; i < 3; i++) {
            ...
            FinancialPrediction pred = financialPredictionRepository
                    .findByTargetDateAndPeriodTypeAndModelVersion(
                            targetDate, PeriodType.MONTHLY, response.model_version())
